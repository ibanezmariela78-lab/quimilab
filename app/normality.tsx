        ...
        >
          <View style={styles.header}>
            <View style={[styles.mark, { backgroundColor: colors.soft }]}>
              <Ionicons name="flash-outline" size={25} color={colors.accent} />
            </View>
            <ThemedText style={[styles.title, { color: colors.text }]}>
              Preparar por normalidad
            </ThemedText>
            <ThemedText style={[styles.intro, { color: colors.muted }]}>
              La normalidad expresa equivalentes de soluto por litro de solución
              y depende de la reacción química considerada.
            </ThemedText>
          </View>
          ...
          >
            ...
            />
            <Field
              label="Normalidad deseada"
              value={normality}
              onChangeText={setNormality}
              placeholder="0,5"
            ...
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel="Calcular preparación por normalidad"
              style={({ pressed }) => [
                styles.button,
  ...
  return (
    <View style={styles.resultArea}>
      <View style={[styles.resultCard, { backgroundColor: colors.soft }]}>
        ...
        <ThemedText style={[styles.body, { color: colors.text }]}>
          {result.equivalenceFactor} · ácido-base
        </ThemedText>
        <ThemedText style={[styles.detailLabel, { color: colors.accent }]}>
          Peso equivalente
        </ThemedText>
        <ThemedText style={[styles.mass, { color: colors.text }]}>
          {format(result.equivalentWeight)} g/eq
        </ThemedText>
      </View>
      <View
        style={[
      ...
      >
        ...
        <ThemedText style={[styles.body, { color: colors.muted }]}>
          N = {format(result.normality)} eq/L
        </ThemedText>
        <ThemedText style={[styles.body, { color: colors.muted }]}>
        ...
        <ThemedText style={[styles.body, { color: colors.muted }]}>
          M = N / factor = {format(result.normality)} /{" "}
          {result.equivalenceFactor} = {format(result.molarity)} M
        </ThemedText>
        <ThemedText style={[styles.sectionTitle, { color: colors.text }]}>
        ...
        <ThemedText style={[styles.sectionTitle, { color: colors.text }]}>
          Paso 2: Determinar peso equivalente
        </ThemedText>
        <ThemedText style={[styles.equation, { color: colors.text }]}>
          PE = MM / factor{`\n`}PE = {format(result.molarMass.molarMass ?? 0)} /{" "}
          {result.equivalenceFactor} = {format(result.equivalentWeight)} g/eq
        </ThemedText>
        <ThemedText style={[styles.sectionTitle, { color: colors.text }]}>
          Paso 3: Calcular masa necesaria
        </ThemedText>
        <ThemedText style={[styles.equation, { color: colors.text }]}>
          masa = N × V × PE{`\n`}masa = {format(result.normality)} ×{" "}
          {format(result.volumeLiters, 3)} × {format(result.equivalentWeight)}
          {`\n`}masa = {format(result.gramsNeeded)} g
        </ThemedText>
        <ThemedText style={[styles.explanation, { color: colors.muted }]}>
          {result.equivalenceExplanation} La normalidad no es una propiedad fija
          de una sustancia: depende de la reacción considerada.
        </ThemedText>
      </View>
      ...
      >
        ...
        />
        <ThemedText style={[styles.body, { color: colors.text }]}>
          La normalidad se basa en equivalentes por litro de solución. El factor
          usado es válido para el cálculo ácido-base indicado.
        </ThemedText>
      </View>
      {observation ? (
        <View
...
const styles = StyleSheet.create({
  ...
  mass: { fontSize: 24, fontWeight: "800", marginBottom: 8, marginTop: 4 },
  sectionTitle: {
    fontSize: 17,
    fontWeight: "700",
    marginBottom: 12,
  ...
  explanation: { fontSize: 14, lineHeight: 21, marginTop: 8 },
  finalMass: {
    fontSize: 20,
    fontWeight: "800",
    lineHeight: 27,
